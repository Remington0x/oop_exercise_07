...
public:
	...
	void LoadFromFile(FILE* in) {
		...
		for (size_t i = 0; i < n; ++i) {
			...
			if (type == QUAD_TYPE_ID) {
				FiguresList.push_back(TFactory< SCALAR_TYPE, TQuad<SCALAR_TYPE> >::Read(in));
			} else if (type == RECTANGLE_TYPE_ID) {
				FiguresList.push_back(TFactory< SCALAR_TYPE, TRectangle<SCALAR_TYPE> >::Read(in));
			} else if (type == TRIANGLE_TYPE_ID) {
				FiguresList.push_back(TFactory< SCALAR_TYPE, TTriangle<SCALAR_TYPE> >::Read(in));
			}
		}
	}
	...
	void Add(const size_t & pos, const unsigned int & figureID) {
		if (figureID == QUAD_TYPE_ID) {
			AddFigure(pos, TFactory<SCALAR_TYPE, TQuad<SCALAR_TYPE> >::CreateFigure());
		} else if (figureID == RECTANGLE_TYPE_ID) {
			AddFigure(pos, TFactory<SCALAR_TYPE, TRectangle<SCALAR_TYPE> >::CreateFigure());
		} else if (figureID == TRIANGLE_TYPE_ID) {
			AddFigure(pos, TFactory<SCALAR_TYPE, TTriangle<SCALAR_TYPE> >::CreateFigure());
		}
	}
